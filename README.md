...
## Credits
This program would not be possible if it was not for the amazing wealth of knowledge from www.thinkwiki.org.  Please consider contributing or donating to this page.
Acknowledgment is also due to www.gnu.org and www.fsf.org for all their amazing work with the GNU license and Free Software.  Please consider donating or becoming a member of the FSF!

## Usage
### Modes
- Normal: Run program as root or by running it with "sudo" for use of all features.  Alternatively, users can set ownership to "root" and enable the "setuid" bit on file permissions, as follows: *NOTE: GNOME does not generally support using the "setuid" executing with root privileges*
```
chown root pkf_trackpoint-qt
chmod u+s pkf_trackpoint-qt
```
...
- To enable scrolling on a distribution that uses "Evdev" for device management, click the checkbox at the button of the screen titled "Enable Scrolling"  
*NOTE: This checkbox will only be visible if your distribution is using "Evdev".  To determine what your distribution is using, run the following*
```bash
xinput list-props "TPPS/2 IBM TrackPoint" | grep Evdev
```   
